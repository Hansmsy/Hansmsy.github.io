...
## 旧架构全跑一遍，反而留下了一份完美的训练集

旧架构把所有subagent都跑一遍，慢且贵。但正因为它不做选择，历史数据是一份**完整特征矩阵加事后真标签**——每个账户的每个特征都有值，没有缺失、没有选择偏差。

<div class="claim" markdown="1">
旧架构一次性付出的全量成本，换来了之后长期的稀疏采样能力：**用这份数据离线算出每个特征的判别力，之后就不必再全跑。**
</div>
...
互信息的经验估计在**样本少时系统性偏高**——格子里样本不足会虚增判别力，因此要把估计值往零收缩。这与VR-OPD里的正确性门控收缩是同一个判断：**证据弱就退回基线**，只是这次的基线是「没有信息量」。
</div>
...
反爬是**对抗场景**，爬虫会主动演化，今天没有判别力的特征明天可能变成最强特征，因此边权必须周期性重估。但新架构自己带来了一个陷阱：

<div class="claim" markdown="1">
旧架构全跑，所以数据完整、边权可以无偏估计。**新架构只跑Top-N，新数据就只有被选中的特征有值——而「被选中」是图谱自己决定的，缺失不是随机的。**
</div>

后果是图谱会**自我固化**：它认为不重要的特征永远不再被采集，也就永远没有机会被重新评估。在对抗场景里这一点是致命的。

解法是保留一小部分流量继续全跑 (约1%)，专门作为无偏的重估数据源。所以增量维护的动作不是「把新数据并进去」，而是**用那部分全量采样流量周期性重估条件信息增益**。

<div class="echo" markdown="1">
<span class="echo-tag">↔ 与我的论文呼应</span>
这与MCPO里轮次级轨迹剪枝要治的病是同一个：**抑制自我强化的曝光偏置**。区别只在于MCPO作用于策略的采样，这里作用于数据的采集。
</div>
