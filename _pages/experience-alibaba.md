...
图谱以**步骤为节点、步骤之间的共性关系为边**，边的来源分在线与离线两路构建：

<div class="cards" markdown="1">
<div class="card"><span class="card-t">在线构建</span><span class="card-d">提取旧Agent上真实执行过的轨迹，筛选出成功轨迹后归纳步骤间的共性关系；边权按共现概率赋值，<b>共现频次越高权重越大</b></span></div>
<div class="card"><span class="card-t">离线构建</span><span class="card-d">针对长尾与低频步骤人工合成轨迹，送入Agent实际执行验证，<b>验证通过后方可并入图谱</b></span></div>
</div>

两路分工明确：在线构建保证图谱贴合真实分布，但真实流量中难以采集到的长尾无法覆盖；离线构建负责补齐这部分，且合成轨迹必须通过实际执行验证方可并入，这是一条硬性约束。

### ② 计算边权

起点就是转移频率——$u$之后走过$v$的次数，占$u$之后所有走法的比例：
...
因为$w$只是计数的函数，**任何轨迹的并入都只是加计数**，不需要重建图谱。三类来源走同一个入口：成功过滤→切成相邻步骤对→更新计数，区别只在加多少：

| 来源 | 准入门槛 | 加的计数 |
| :-- | :-- | :-- |
| 在线轨迹 | 真实执行成功 | $+1$ |
| 离线合成轨迹 | 送入Agent实际执行验证通过 | 一次性保底计数 |

需要维护的只有四个计数器：$c(u \to v)$、$c(u)$、$c(v)$与总数，权重在检索时按需计算。因此一条轨迹的并入代价只与该轨迹的长度成正比，与图谱规模无关——这是增量维护在工程上可行的前提。

离线边只执行验证过一次，若也只加1，它就是表中$v_3$那一档，永远排不上来。所以给它一次性的保底计数，让它一进图谱就具备中等支撑度。但**保底计数是试用期，不是永久席位**，后续由真实反馈结算：被选中并执行成功则转为正常在线计数、保底撤销；反复失败或长期从未被选中则撤销保底，退回长尾池。

<div class="claim" markdown="1">
所以并不是「相信人工构造的数据」，而是**只给它一次被真实流量检验的机会**。
</div>
