...
<div class="pipe" markdown="1">
<div class="pipe-step"><span class="pipe-tag">STEP 1</span><span class="pipe-name">构建图谱</span><span class="pipe-desc">步骤作节点、步骤间的共性关系作边，在线与离线两路</span></div>
<div class="pipe-step"><span class="pipe-tag">STEP 2</span><span class="pipe-name">计算边权</span><span class="pipe-desc">转移频率经平滑与去热门两次修正，得到提升度</span></div>
<div class="pipe-step"><span class="pipe-tag">STEP 3</span><span class="pipe-name">检索排序</span><span class="pipe-desc">结合边权与任务语义排序，只取Top-N进上下文</span></div>
<div class="pipe-step"><span class="pipe-tag">STEP 4</span><span class="pipe-name">增量维护</span><span class="pipe-desc">新轨迹按同一入口并入，过回归门控后生效</span></div>
</div>

### ① 构建图谱

图谱以**步骤为节点、步骤之间的共性关系为边**，边的来源分在线与离线两路：

<div class="cards" markdown="1">
<div class="card"><span class="card-t">在线构建</span><span class="card-d">提取旧Agent上真实执行过的轨迹，筛选出成功轨迹后归纳步骤间的共性关系。<b>边权只由这一路的计数决定</b></span></div>
<div class="card"><span class="card-t">离线定向采样</span><span class="card-d">人工只构造长尾场景的<b>任务</b>，由同一个Agent在同样的工具与环境下自主执行，仅保留真实成功的轨迹。<b>它补的是图谱的结构覆盖，不参与边权计算</b></span></div>
</div>

两路的定位必须分清，否则会破坏边权的统计意义：

<div class="claim" markdown="1">
**离线扩充的是图谱的覆盖面，不是边权的置信度。**
</div>

离线这一路的人工介入只发生在**任务的采样**上，不介入Agent的行为——本质是任务空间上的**分层过采样**，对低频分层过采样，而Agent的行为分布保持不变。这与人工编造轨迹是两件不同的事：后者等于替Agent写答案。

<div class="echo" markdown="1">
<span class="echo-tag">↔ 反馈闭环</span>
纯靠在线日志构建图谱存在固有的**反馈闭环**：没被走过的边拿不到证据，拿不到证据就进不了候选，进不了候选就永远走不到。离线定向采样提供的是**外生的探索信号**，用来打破这个闭环。
</div>

### ② 计算边权

起点就是转移频率——$u$之后走过$v$的次数，占$u$之后所有走法的比例。**这里的计数只来自真实在线流量**：

$$w(u \to v) = \frac{c(u \to v)}{c(u)}$$

它有两个偏差，各修一次。

**修正一：证据不足就不应自信。** 一条边只出现过1次，不应与出现过500次的边一样自信。因此掺入全局平均：样本少时依赖全局，样本足够时才相信局部。

$$\hat{p}(v \mid u) = \frac{c(u \to v) + \alpha\, p(v)}{c(u) + \alpha}$$

$\alpha$只有一个含义：**需要积累约$\alpha$份证据，才开始相信局部统计**。

<div class="echo" markdown="1">
<span class="echo-tag">↔ 与我的论文呼应</span>
这与VR-OPD里的正确性门控收缩是同一个判断：**证据弱就退回基线**。
</div>

**修正二：热门不等于合适。** 有些步骤跟在任何步骤后面都常见，频率高但没有信息量。该问的不是「$v$常不常见」，而是「$v$在$u$之后，比它平常更常见吗」：

$$w(u \to v) = \frac{\hat{p}(v \mid u)}{p(v)}$$

这个量的标准名字是**提升度 (lift)**，含义就是「比平常提升了几倍」。$w > 1$说明$u$之后$v$确实变得更可能，$w < 1$说明反而更少见。

<div class="echo" markdown="1">
<span class="echo-tag">↔ 与我的论文呼应</span>
这正是MCPO里轮次级轨迹剪枝要治的病：**抑制热门候选的曝光偏置，把机会让给稀有候选**。
</div>

两步合起来是一个式子：

$$w(u \to v) = \frac{1}{p(v)} \cdot \frac{c(u \to v) + \alpha\, p(v)}{c(u) + \alpha}$$

取$c(u) = 541$、$\alpha = 8$举例：

| 候选 | 出现次数 | 全局热度 | 纯频率 | 提升度 |
| :-- | --: | --: | --: | --: |
| $v_1$ 通用热门步骤 | 500 | 30% | 92.4% | 3.05 |
| $v_2$ 专用步骤 | 40 | 2% | 7.4% | **3.66** |
| $v_3$ 只出现一次 | 1 | 1% | 0.2% | 0.20 |

这张表里有两个反转：纯频率下$v_1$领先$v_2$达**12.5倍**，而按提升度**$v_2$反而更相关**；但$v_3$并没有跟着翻上来，提升度只有0.20。

<div class="claim" markdown="1">
两个修正是**互相制衡**的：修正二防止热门霸占名额，修正一防止冷门虚高。
</div>

### ③ 检索排序

边权是任务无关的——同一个节点，无论本次任务是什么，候选顺序都一样。所以决策时用两项：

$$\text{score} = \lambda \cdot w(u \to v) + (1 - \lambda) \cdot \mathrm{sim}(q,\, v)$$

其中$q$是当前任务的描述。**图谱负责判断这条路走得通，语义负责判断它与本次任务有关。** $\lambda$取约0.6，结构主导、语义重排。

只把score最高的Top-N放进上下文。这一步直接针对上一屏的三个后果：进入上下文的不再是全部可行路径，而是一个**固定大小**的候选集，上下文规模因此与可行路径总数解耦，开放式动作生成也收敛为候选路径选择。

Top-N里还预留$k$个席位给零证据或低证据的边 ($N = 5$、$k = 1$)。这不是给它们更高的权重，而是给它们**一次被真实流量检验的曝光机会**——缺曝光就补曝光，而不是补假证据。

<div class="echo" markdown="1">
<span class="echo-tag">↔ 与我的论文呼应</span>
把探索导向稀有候选，正是MCPO里轮次级轨迹剪枝在做的事。离线定向采样与探索名额，是同一个探索命题在**数据侧与排序侧**的两个解法。
</div>

### ④ 增量维护

因为$w$只是计数的函数，**在线轨迹的并入就只是加计数**，不需要重建图谱：成功过滤→切成相邻步骤对→计数加一。需要维护的只有四个计数器：$c(u \to v)$、$c(u)$、$c(v)$与总数，权重在检索时按需计算。因此一条轨迹的并入代价只与该轨迹的长度成正比，与图谱规模无关——这是增量维护在工程上可行的前提。
...
| :-- | :-- | :-- |
| 在线轨迹 | **证据**（计数加一） | 直接决定边权 |
| 离线定向采样 | **结构**（这条边存在） | 不参与边权计算 |

所以新边入图时计数为零，靠检索排序里的探索名额拿到第一次曝光；之后它的权重完全由真实流量给出。长期获得曝光但始终没有成功记录的边会自然沉底，不需要额外的淘汰逻辑。

并入按批进行而非逐条实时写入：汇总得到候选新图谱后，先在历史轨迹上离线回放，**关键路径召回率Recall@N不下降才生效，否则回滚**。

<div class="echo" markdown="1">
<span class="echo-tag">↔ 同一条工程纪律</span>
这与04屏Skill自进化里「验证是强制门控」是同一条原则：**任何自动产生的资产，进生产前必须过一次自动化回归**。同一个Recall@N既用来选超参，也用来守回归。
</div>
